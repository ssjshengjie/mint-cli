...
// lib/utils/renderMain.ts
var import_fs = __toESM(require("fs"), 1);
var renderMain = (ts, router, pinia, root) => {
  const template = `
  ...
  const path3 = `${root}/src/${name}`;
  import_fs.default.writeFileSync(path3, JSON.stringify(template, null, 2) + "\n");
};
...
var renderTemplate = (src, dest) => {
  const stats = import_fs2.default.statSync(src);
  ...
  if (filename === "package.json" && import_fs2.default.existsSync(dest)) {
    const existing = JSON.parse(import_fs2.default.readFileSync(dest, "utf8"));
    const newPackage = JSON.parse(import_fs2.default.readFileSync(src, "utf8"));
...
var clg_default = banner("MINT CLI Vue.js - The Progressive JavaScript Framework");
...
if (options2.dest)
  console.log(options2.dest);
if (options2.frame)
  console.log(options2.frame);
